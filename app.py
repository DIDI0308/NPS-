...
st.markdown("""
    ...
    .emoji-yellow {
        font-size: 80px;
        text-align: center;
        color: #FFD700; /* Amarillo para la carita */
        line-height: 1;
        margin-bottom: 10px;
    }
    /* Estilo para los inputs de Streamlit dentro de las tarjetas */
    div[data-baseweb="input"] { background-color: #f9f9f9 !important; }
    label { color: #333333 !important; font-weight: bold !important; }
    
    /* Simulación del header naranja para el Driver */
    .driver-header input {
...
# ==========================================
# 4. FILA DINÁMICA B (SCORE PROMEDIO)
# ==========================================
...
# ==========================================
st.markdown("<hr style='border: 1px solid #333;'>", unsafe_allow_html=True)
st.markdown('<p style="color:white; font-size:30px; font-weight:bold; text-align:center;">COMMENTS CHOSEN</p>', unsafe_allow_html=True)

col_t1, col_t2, col_t3 = st.columns(3)

def render_editable_card(col, key_id, default_title):
    with col:
        st.markdown(f'<div class="card-white"><div class="emoji-yellow">☹</div>', unsafe_allow_html=True)
        
        # El título del Driver con fondo naranja se simula con un input de texto
        st.text_input("Driver:", value=default_title, key=f"header_{key_id}", label_visibility="collapsed")
        
        # Campos de datos
        st.text_input("Cliente:", key=f"client_{key_id}")
        st.number_input("Score:", min_value=0, max_value=10, key=f"score_{key_id}")
        st.text_area("Comentario:", key=f"comment_{key_id}", height=100)
        st.text_input("Camión:", key=f"truck_{key_id}")
        
        st.markdown('</div>', unsafe_allow_html=True)

render_editable_card(col_t1, 1, "Secondary Driver 1:")
render_editable_card(col_t2, 2, "Secondary Driver 2:")
render_editable_card(col_t3, 3, "Secondary Driver 3:")
