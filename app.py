        ...
        if not df_coords.empty:
            ...
            if not df_map.empty:
                ...
                fig_map = px.density_mapbox(
                    ...
                    zoom=15 if busqueda else 11, mapbox_style="open-street-map",
                    hover_name='Customer ID', hover_data={'Score': True, 'Category': True, 'Lat': False, 'Lon': False},
                ...
                st.info("No se encontró el cliente o no tiene coordenadas registradas en la base.")
...
# ==========================================
    ...
    if not df_raw_evo.empty:
        ...
        for r in rows_det:
            ...
            for c in range(3, 15): table_html += f'<td style="color:black;">{df_raw_evo.iloc[r, c] if pd.notnull(df_raw_evo.iloc[r, c]) else "-"}</td>'
            table_html += '</tr>'
...
# ==========================================
    ...
    st.markdown("""
        ...
        .banner-amarillo { background-color: #FFFF00; padding: 15px; display: flex; justify-content: center; align-items: center; border-radius: 5px; margin-bottom: 25px; }
        .banner-amarillo h1 { color: black; margin: 0; font-family: 'Arial Black', sans-serif; }
        </style>
        """, unsafe_allow_html=True)
    
    if st.button("⬅ VOLVER AL INICIO", key="back_btn_ea"):
        st.session_state.page = "home"
        st.rerun()
        
    st.markdown('<div class="banner-amarillo"><h1>EA / LP PERFORMANCE</h1></div>', unsafe_allow_html=True)
    st.write("Contenido EA/LP próximamente...")
