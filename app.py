...
st.markdown(f"""
    ...
    /* Estilo para que la imagen ocupe TODA la pestaña en la Landing */
    {" .stApp { background-image: url('data:image/png;base64," + b64_bg + "'); background-size: cover; background-position: center; background-repeat: no-repeat; background-attachment: fixed; }" if st.session_state.page == 'landing' and b64_bg else ""}

    /* Eliminar márgenes y padding de Streamlit */
    ...
    div.stButton > button:hover {{
        ...
        box-shadow: 0px 15px 25px rgba(255, 255, 0, 0.3);
    }}
...
if st.session_state.page == 'landing':
    if not b64_bg:
        st.error("Archivo 'logo3.png' no encontrado. Por favor asegúrate de que esté en la raíz.")
    
    # Espaciado para bajar los botones a la zona de los cuadros amarillos de la imagen
    # Ajusta este 'height' si los botones quedan muy arriba o muy abajo
    st.markdown('<div style="height: 78vh;"></div>', unsafe_allow_html=True)
    
    col_l, col_btn1, col_gap, col_btn2, col_r = st.columns([1.5, 3, 1, 3, 1.5])
...
# ==========================================
    ...
    # Contenedor para el análisis con padding para no chocar con bordes
    container = st.container()
    with container:
        if st.button("⬅ VOLVER AL INICIO", key="back_curr"):
            st.session_state.page = 'landing'
            st.rerun()
