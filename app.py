...
# ==========================================
    ...
    if st.button("⬅ VOLVER AL INICIO", key="back_btn_m"): st.session_state.page = "home"; st.rerun()
    ...
    if not df_raw.empty:
        def render_nps_block(df, row_start_idx, title_prefix):
            ...
            st.markdown(f"""<div class="section-banner"><h2 style='color: black; margin: 0; font-size: 19px;'>
                        {title_prefix} | {int(y25_m[last_idx])} {mes_txt} – {int(y24_m[last_idx])} LY | {int(val_ytd_25)} YTD vs {int(val_ytd_bu)} BGT</h2></div>""", unsafe_allow_html=True)
            ...
            with ca:
                ...
                fig_l.update_layout(paper_bgcolor='black', plot_bgcolor='black', font=dict(color="white"), xaxis=dict(showgrid=False), yaxis=dict(visible=False, range=[min(y25_m+y24_m)-10, max(y25_m+y24_m)+20]), height=400)
                st.plotly_chart(fig_l, use_container_width=True)
            with cb:
                fig_b = go.Figure()
                fig_b.add_trace(go.Bar(x=["2024", "BGT", "2025"], y=[val_ytd_24, val_ytd_bu, val_ytd_25], text=[f"{val_ytd_24}", f"{val_ytd_bu}", f"{val_ytd_25}"], marker_color=['#F4D03F', '#FFD700', '#FFFF00'], width=0.6, textfont=dict(color="black", size=14, family="Arial Black")))
        ...
        for r in rows_det:
            table_html += f'<tr><td style="font-weight:bold; text-align:left;">{str(df_raw.iloc[r, 0])}</td>' + "".join([f'<td>{df_raw.iloc[r, c] if pd.notnull(df_raw.iloc[r, c]) else "-"}</td>' for c in range(3, 15)]) + '</tr>'
        ...
        for idx, col in zip([18, 20, 22], [col_a1, col_a2, col_a3]):
            ...
            palabras = txt_orig.split(); mitad = len(palabras) // 2; txt_form = "<br>".join([" ".join(palabras[:mitad]), " ".join(palabras[mitad:])])
            ...
            fig_ring.add_annotation(text=f"<b>{txt_form}</b>", x=0.5, y=-0.25, showarrow=False, font=dict(color="white", size=14))
...
# ==========================================
    ...
    if st.button("⬅ VOLVER AL INICIO", key="back_btn_ea"): st.session_state.page = "home"; st.rerun()
