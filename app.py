...
# ==========================================
    ...
    if not df.empty:
        ...
        if not df_sec.empty:
            ...
            fig5.update_layout(title={'text': "5. Avg Score by Secondary Driver", 'x': 0.5, 'xanchor': 'center', 'font': font_main}, barmode='overlay', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', yaxis=dict(visible=False, range=[0, 12]), height=650)
...
# ==========================================
    ...
    if not df_raw.empty:
        ...
        for idx, col in zip(indices_ytd, [col_a1, col_a2, col_a3]):
            valor_ytd, texto_original = df_raw.iloc[idx, 2], str(df_raw.iloc[idx, 0])
