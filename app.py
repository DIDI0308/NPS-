...
# ==========================================
    ...
    if not df.empty:
        ...
        # --- MAPA DE CALOR CON BUSCADOR DINÁMICO ---
...
# ==========================================
    ...
    st.markdown(f"""
        <div class="header-banner">
            <img src="data:image/png;base64,{izq if izq else ""}" height="70">
            <h1 class="header-title">MONTHLY EVOLUTION</h1>
            <img src="data:image/png;base64,{der if der else ""}" height="70">
        </div>
        """, unsafe_allow_html=True)
    ...
    if not df_raw.empty:
        def render_nps_block(df, row_start_idx, title_prefix):
            ...
            st.markdown(f"""<div class="section-banner"><h2 style='color: black; margin: 0; font-size: 19px;'>
                        {title_prefix} | {int(y25_m[last_idx])} {meses[last_idx]} – {int(y24_m[last_idx])} LY | {int(val_ytd_25)} YTD vs {int(val_ytd_bu)} BGT YTD</h2></div>""", unsafe_allow_html=True)
            ...
            with col_a:
                ...
                fig_l.update_layout(paper_bgcolor='black', plot_bgcolor='black', font=dict(color="white"), xaxis=dict(showgrid=False, tickfont=dict(color="white")), yaxis=dict(visible=False, range=[min(y25_m+y24_m)-15, max(y25_m+y24_m)+25]), height=500, margin=dict(t=50))
        ...
        for r in rows_det:
            table_html += f'<tr><td style="font-weight:bold; color:black;">{str(df_raw.iloc[r, 0])}</td>'
            for c in range(3, 15): table_html += f'<td style="color:black;">{df_raw.iloc[r, c] if pd.notnull(df_raw.iloc[r, c]) else "-"}</td>'
            table_html += '</tr>'
